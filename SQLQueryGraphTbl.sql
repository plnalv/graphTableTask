...
WHERE MATCH(Artist1-(Featuring)->Artist2)
 AND Artist1.name = N'Miley Cyrus'; --кто соавтор соавтора Ariana Grande? SELECT artist1.name + N' имеет фит с ' + artist2.name AS Level1
 , artist2.name + N' имеет фит с ' + artist3.name AS Level2
FROM Artist AS artist1
...
-- все песни, которые входят в альбомы, выпущенные после определенного года, и их авторов:
SELECT album.name AS album
     , song.name AS song
     , artist.name AS artist
FROM Album AS album
...
--артисты, которые исполняют песни определенного жанра:
